// Package provider_test validates the contract every provider must satisfy.
//
// These are contract tests rather than per-adapter tests: each provider is
// checked for a stable name, the configuration key it claims, correct decoding
// of its own section, and a Close that is safe on a provider that was never
// initialised. A provider that violates any of these breaks the engine in a way
// that only shows up at a consumer's startup.
...
func (stubLogger) Info(context.Context, string, map[string]any)  {}
func (stubLogger) Error(context.Context, error, map[string]any)  {}
func (stubLogger) Debug(context.Context, string, map[string]any) {}
func (stubLogger) Warn(context.Context, string, map[string]any)  {}
