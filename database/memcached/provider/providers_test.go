// Package provider_test verifies the Memcached provider contract in the module
...
func TestConfigKey(t *testing.T) {
	assert.Equal(t, "memcached_clients", memcached.New("x").ConfigKey())
}

func TestNameIsNamespaced(t *testing.T) {
	p := memcached.New("cache")
...
func TestUnknownKeyIsReported(t *testing.T) {
	p := memcached.New("cache")
	raw := section(p.ConfigKey(), "cache", map[string]any{"serverz": "x"})

	_, err := p.Init(context.Background(), raw, deps())

	require.Error(t, err)
	assert.ErrorContains(t, err, "serverz")
}
...
func TestUnreachableTargetIsReported(t *testing.T) {
	p := memcached.New("cache")
	raw := section(p.ConfigKey(), "cache", map[string]any{"servers": []any{"127.0.0.1:1"}})
