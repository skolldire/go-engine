// Package provider_test verifies the MongoDB provider contract in the module
...
func TestConfigKey(t *testing.T) {
	assert.Equal(t, "mongodb_clients", mongodb.New("x").ConfigKey())
}
...
func TestUnknownKeyIsReported(t *testing.T) {
	p := mongodb.New("cache")
	raw := section(p.ConfigKey(), "cache", map[string]any{"ur": "x"})

	_, err := p.Init(context.Background(), raw, deps())

	require.Error(t, err)
	assert.ErrorContains(t, err, "ur")
}
...
func TestUnreachableTargetIsReported(t *testing.T) {
	p := mongodb.New("cache")
	raw := section(p.ConfigKey(), "cache", map[string]any{"uri": "mongodb://127.0.0.1:1", "timeout": "1s"})
