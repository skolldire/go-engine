...
- **`sqlc` provider** (`database/sqlc/provider/sqlc`): reads `sqlc_clients`, registers as `sqlc:<instance>` and contributes a health check. Unlike the GORM provider nothing is injected in Go — the driver is a name in the YAML backed by a blank import (`_ "github.com/jackc/pgx/v5/stdlib"`), with `WithConnector` for credentials a DSN cannot carry. The two SQL providers use different sections and different component names, so an application chooses one when it assembles the engine, or registers both.
