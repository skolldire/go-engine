...
// TestDeprecatedWithResilienceStillWorks keeps existing configuration valid.
func TestDeprecatedWithResilienceStillWorks(t *testing.T) {
	srv, calls := statusServer(t, http.StatusServiceUnavailable)

	//nolint:staticcheck // exercising the deprecated field on purpose
	c := NewClient(Config{BaseURL: srv.URL, WithResilience: true}, &testutil.MockLogger{})
	_, err := c.Get(context.Background(), "/x", nil)

	require.Error(t, err)
	assert.Greater(t, calls.Load(), int64(1), "the legacy flag must still enable retries")
}
